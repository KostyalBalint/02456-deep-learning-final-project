...
class MethodResults:
    ...
    def to_dict(self, include_arrays=False) -> Dict:
        ...
        if include_arrays:
            ...
            # Maps are too large for JSON, handle separately
    ...
    def load(cls, path: Path) -> 'MethodResults':
        ...
        maps = [arrays[f'map_{i}'] for i in range(len([k for k in arrays.keys() if k.startswith('map_')]))]
...
class ExperimentResults:
    ...
    def per_object_table(self) -> pd.DataFrame:
        """Create detailed per-object results table."""
        rows = []
