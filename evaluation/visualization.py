...
def plot_single_result(
    ...
    if show_prediction_mask:
        ...
        axes[plot_idx].axis('off')
        axes[plot_idx].set_title(f'Predicted Mask\n({segmentation_method})', fontsize=12, fontweight='bold')
        plot_idx += 1
...
def plot_result_grid(
    ...
    for idx in selected_indices:
        plot_single_result(
            ...
            segmentation_method=results.config.get('segmentation_method'),
...
def visualize_results(test_image,
                      anomaly_map,
                      segmentation_method,
...
def visualize_multiple_results(images,
                               anomaly_maps,
                               scores,
                               labels,
                               segmentation_method,
    ...
    for idx in selected_indices:
        ...
        visualize_results(images[idx], anomaly_maps[idx], segmentation_method, ground_truth_mask = gt_mask,
                         image_score = scores[idx], label = labels[idx], sigma=sigma,
                         vmin=vmin, vmax=vmax,
